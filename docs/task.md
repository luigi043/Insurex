...
- [ ] Create an immutable Audit Log / Evidence Ledger (capturing who, what, when, old/new values, and `CorrelationId`).
- [ ] Implement Case Management and Workflow Orchestration (tasks, SLA timers, escalations).
- [ ] Refactor the [Notification_Provider](file:///c:/Users/cluiz/projectx/Insurex/IAPR_Data/Providers/Notification_Provider.cs#19-814) to send targeted, templated emails/SMS securely with a robust retry policy (addressing the current bulk BCC/CC leaks).
